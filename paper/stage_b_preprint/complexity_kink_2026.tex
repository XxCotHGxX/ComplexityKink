...
\begin{center}
\fbox{\parbox{0.92\columnwidth}{\small\textbf{Repository note.} This preprint reports the Stage~B analysis (IV/Random Forest instruments). The companion repository at \url{https://github.com/mhernandez2026/ComplexityKinkResearch} implements Stage~C (LLM rubric instruments), which is the basis of the forthcoming revision. Pre-registered predictions for the Stage~C rerun are recorded in \texttt{docs/predictions\_and\_rewrite\_notes.md}.}}
\end{center}
...
Below $\hat{\gamma} = 6.5$, mean pass rates are 40.4\% ($n = 19{,}458$). Above it, they drop to 11.8\% ($n = 16{,}041$)---a difference of 28.6 percentage points. This pattern is consistent with a structural break in the complexity--performance relationship rather than a gradual decline. Additionally, the Wald statistic exhibits a secondary peak at $\hat{\gamma} \approx 13.5$ (Figure~\ref{fig:hansen}), suggesting a multi-stage collapse where reliability further deteriorates into a 'Zone of Incapability' beyond this second threshold.
...
\textbf{Reproducibility.} All code and analysis scripts are available at: \url{https://github.com/XxCotHGxX/ComplexityKink}.
