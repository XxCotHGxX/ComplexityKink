...
\caption{Per-model threshold-search diagnostic, not a pass-rate plot. Rows are the 21 panel models and columns are candidate complexity thresholds $\gamma$. Each cell reports the Hansen Wald statistic $W(\gamma)$ for placing the structural break at that candidate threshold; larger values mean stronger evidence for a kink at that $\gamma$. The marker identifies each row's maximum, which is that model's estimated threshold $\hat{\gamma}$.}
\label{fig:heatmap}
\end{figure}
