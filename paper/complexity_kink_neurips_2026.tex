...
We first explored this identification problem with keyword features extracted from prompts, using them as instruments for latent target complexity. That pilot analysis motivated the present design but is not treated as external evidence here. Keyword instruments are limited: they are coarse proxies for program structure, can be non-monotone in true complexity, and provide weak discrimination among low-complexity tasks where output-side contamination is most concentrated. The present paper replaces that proxy with a direct prompt-side measurement design.
...
\subsection{Placebo}
As an additional robustness check, we randomly permute $\hat{\kappa}$ across observations 500 times, destroying any true threshold structure, and recompute $W^*$ at each permutation. The observed $W^*$ should lie in the far tail of the placebo distribution if the kink is real. Figure~\ref{fig:pipeline} summarizes the end-to-end measurement and estimation workflow.

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{pipeline.png}
\caption{Analysis pipeline. Prompts are scored by out-of-panel judges against the six dimension rubric. Rubric scores enter 2SLS as instruments for observed complexity. Downstream tests are computed on the rubric-instrumented fit.}
\label{fig:pipeline}
\end{figure}
...
A critical failure of the keyword-instrument pilot was its inability to separate $\kappa=1$ from $\kappa=2$ prompts. The distinction matters because $\kappa=1$ is where endogenous contamination concentrates. Table~\ref{tab:low-end-separation} contrasts the pilot keyword features against the prompt-side rubric composite on this specific discrimination.

\begin{table}[h]
\centering
\caption{The prompt-side rubric discriminates $\kappa=1$ from $\kappa=2$ at $p<10^{-6}$. Pilot keyword features did not achieve statistical separation.}
...
We therefore do not treat the $p$-value as dispositive. The cautionary reading is that our exclusion argument rests on design rather than on test, and we flag scorer idiosyncrasy as a possible contributor to the rejection. The ensemble design is intended to mitigate this risk by averaging across out-of-panel judges and exposing scorer disagreement directly.
...
Figure~\ref{fig:sankey} visualizes the mechanism underlying the benchmark bias. Tasks flow from their true (rubric) complexity on the right to their observed output CC bin on the left. Red flows are tasks that are complex by rubric but register as $\kappa^{\text{obs}}=1$ by output, which is the contamination that drives the Reverse Threshold Problem. Approximately 2\% of the dataset routes through the red flow, illustrating the bias that the prompt-side rubric instrument isolates.
...
\subsection{Elimination of Pilot Instrument Weaknesses}

Table~\ref{tab:b-vs-c} compares the pilot keyword instrument and the prompt-side rubric on the main measurement weaknesses.

\begin{table}[h]
\centering
\caption{Keyword pilot compared with prompt-side LLM rubric on known measurement weaknesses.}
\label{tab:b-vs-c}
\begin{tabular}{@{}lll@{}}
\toprule
Issue & Keyword pilot & Prompt rubric \\
\midrule
